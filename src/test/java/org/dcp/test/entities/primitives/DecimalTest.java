...
public class DecimalTest {
    ...
    public void testReadWriteFunctionality() {
        ...
        //checkSerialization(new DecimalIntegral(true, 1, 1, Decimal.DecimalFormat.IEEE_754_QUADRUPLE_128));
        //checkSerialization(new DecimalIntegral(true, 1, 1, Decimal.DecimalFormat.IEEE_754_OCTUPLE_256));
    }
    ...
    public void testWriteFunctionality() {
        ...
        assertEquals(bitOutputStreamFirst.toString(), "0100000000000101101111110000101010001011000101000101011101101001");

        final BitOutputStream bitOutputStreamSecond = new MockBitOutputStream();
        new Decimal(-Math.E).writeTo(bitOutputStreamSecond);
        assertEquals(bitOutputStreamSecond.toString(), "1100000000000101101111110000101010001011000101000101011101101001");
    }

    @Test
    public void testReadFunctionality() {
        assertEquals(new Decimal().readFrom(new MockBitInputStream("1100000000000101101111110000101010001011000101000101011101101001")), -Math.E);
        assertEquals(new Decimal().readFrom(new MockBitInputStream("0100000000000101101111110000101010001011000101000101011101101001")), Math.E);
    }
