...
public class Decimal implements BitStreamSerializable<Decimal> {
  ...
  public static enum DecimalFormat {
    ...
    public String toString() {
      return String.format(
          "%s SignBits: %d ExponentBits: %d, MantissaBits: %d",
          name(), signBits, exponentBits, mantissaBits);
    }
  }
  ...
  public static class DecimalIntegral {
    ...
    public DecimalIntegral(final long longValue, final DecimalFormat format) {
      ...
      this.exponentValue =
          (longValue >>> format.mantissaBits)
              & EntropyUtil.findMaximumValueRepresentible(format.exponentBits);
      this.mantissaValue =
          (longValue & EntropyUtil.findMaximumValueRepresentible(format.mantissaBits));
      this.format = format;
    }
    ...
    public String toString() {
      return String.format(
          "Sign: %s Exponent: %d Mantissa: %d Format: %s",
          isNegative ? "-" : "+", exponentValue, mantissaValue, format);
    }
  }
  ...
  public static class IntegralDecimal {
    ...
    protected static Float getFloatValue(final DecimalIntegral decimalIntegral) {
      final DecimalFormat format = decimalIntegral.format;
      int intValueRaw = 0;
      if (decimalIntegral.isNegative)
        intValueRaw |= 1L << (format.mantissaBits + format.exponentBits);
    ...
    protected static Double getDoubleValue(final DecimalIntegral decimalIntegral) {
      ...
      long longValueRaw = 0;
      if (decimalIntegral.isNegative)
        longValueRaw |= 1L << (format.mantissaBits + format.exponentBits);
    ...
    @Override
    public boolean equals(final Object other) {
      if (decimalValue != null) return decimalValue.equals(other);
      return decimalIntegral.equals(other);
    }

    @Override
    public String toString() {
      if (decimalValue != null) return String.valueOf(decimalValue);
  ...
  public Decimal(final Number decimalValue) {
    ...
    else throw new IllegalArgumentException("Unsupported Numeric Object Type.");
  }
  ...
  public Decimal readFrom(final BitInputStream bitInputStream) {
    ...
    final long exponentValue =
        new UnsignedInteger(decimalIntegral.format.exponentBits).readFrom(bitInputStream).value();
    final long mantissaValue =
        new UnsignedInteger(decimalIntegral.format.mantissaBits).readFrom(bitInputStream).value();
    final DecimalIntegral decimalIntegralRead =
        new DecimalIntegral(isNegative, exponentValue, mantissaValue, decimalIntegral.format);
  ...
  public void writeTo(final BitOutputStream bitOutputStream) {
    ...
    new UnsignedInteger(decimalIntegral.exponentValue, decimalIntegral.format.exponentBits)
        .writeTo(bitOutputStream);
    new UnsignedInteger(decimalIntegral.mantissaValue, decimalIntegral.format.mantissaBits)
        .writeTo(bitOutputStream);
  }
